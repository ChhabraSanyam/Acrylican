...
import { Post, PostFilters, PostStatus } from "../../types/post";
...
export const PostStatusDashboard: React.FC<PostStatusDashboardProps> = ({
  ...
  // Real-time updates - only when enabled and component is visible
  useEffect(() => {
    // Don't start interval if disabled or still loading initial data
    if (refreshInterval <= 0 || loading) return;

    const interval = setInterval(() => {
      // Only refresh if we're not currently loading and the component is visible
  ...
  }, [refreshInterval, loading, loadPosts]);

  useEffect(() => {
    loadPosts();
  }, [loadPosts, filters, pagination.skip, pagination.limit]);
  ...
  const handleViewPost = (post: Post) => {
    if (post.status === PostStatus.DRAFT && post.platform_specific_content) {
      // Use the new draft viewer for drafts with platform-specific content
      setViewingPost(post);
    } else {
      // Use the existing post select handler for other posts
      onPostSelect?.(post);
    }
  };
              ...
              >
                ...
                <td className="px-6 py-4">
                  <div className="max-w-xs">
                    <div className="flex items-center space-x-2">
                      ...
                      {post.status === PostStatus.DRAFT && post.platform_specific_content && 
                       Object.keys(post.platform_specific_content).length > 0 && (
                        <span 
                          className="inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800"
                          title="Has AI-generated platform-specific content"
                        >
                          AI
                        </span>
                      )}
                ...
                <td className="px-6 py-4">
                  <div className="flex space-x-2">
                    ...
                    >
                      {post.status === PostStatus.DRAFT && post.platform_specific_content ? 'View Draft' : 'View'}
                    </button>
